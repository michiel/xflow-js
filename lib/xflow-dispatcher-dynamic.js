...
function assertValidDispatcher(name, dispatcher) {
  if (!LangUtil.isFunction(
    ...
    throw new Error(
      `assertValidDispatchers : Dispatcher ${name} does not implement 'dispatch' method`
    );
  }
  if (!LangUtil.isFunction(
    ...
    throw new Error(
      `assertValidDispatchers : Dispatcher ${name} does not implement 'dispatchQ' method`
    );
  }
  if (!LangUtil.isNumber(
    ...
    throw new Error(
      `assertValidDispatchers : Dispatcher ${name} does not have a version method`
    );
  }
}
...
class XFlowDispatcherDynamic {
  ...
  constructor(dispatchers={}) {
    this.name         = 'XFlowDispatcherDynamic';
    this.dispatchers  = {};
