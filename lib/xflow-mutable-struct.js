...
import patcher       from './util/jsonpatcher';
...
function assertIsValidParamScope(p) {
  if (['in', 'local', 'out'].indexOf(p) < 0) {
    throw new Error(
      `assertIsValidParam : ${p} is not a valid parameter scope (in, out, local)`
    );
  }
}

function assertIsXvar(xvar) {
  if (!(
    exists(xvar) &&
    exists(xvar.name) &&
    exists(xvar.vtype)
